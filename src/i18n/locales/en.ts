...
const en: ResourceTree = {
  ...
  "navigation": {
    ...
    "updates": {
      "label": "Updates",
      "blurb": "Check, download and apply all updates"
    },
    "settings": {
      "label": "Settings",
      "title": "Settings",
  ...
  "models": {
    ...
    "ui": {
      ...
      "fiveHour": { "start": "Start 5h", "starting": "Starting…", "hint": "Send one tiny Luna request on this account to start its 5-hour usage window. This uses a small amount of quota and does not spend a Reset credit.", "success": "The 5-hour window was triggered and usage was refreshed.", "failed": "Could not trigger the 5-hour window: {{detail}}", "refreshFailed": "The trigger request completed, but usage could not be refreshed: {{detail}}", "autoOn": "5h auto: on", "autoOff": "5h auto: off", "autoHint": "Keep the 5-hour window continuous using current quota and resetAt. Vellum sends one tiny Luna request at minimum effort only after the old window expires and weekly quota remains above its gate; decisions are recorded in exportable logs." },
  ...
  "remote": {
    ...
    "act": {
      ...
      "chatgptAuthorize": "Authorize on remote",
      "chatgptReauthenticate": "Sign in again",
      "executionLogin": "Add Official execution account",
      "executionFollowControl": "Make model requests follow the Remote Control account",
    ...
    "blocker": {
      ...
      "officialAccountReauthenticationRequired": "The remote ChatGPT credential has expired. Sign in again.",
    ...
    "account": {
      ...
      "catalogHint": "Desktop accounts and their authorization state on this host",
      "chatgptUnavailable": "Cannot read",
      "chatgpt": {
        "ready": "Remote credential ready",
        "synchronized": "Synchronized",
        "pairingRequired": "Needs pairing",
        "pairingPending": "Pairing",
        "activationRequired": "Needs activation",
        "reauthenticationRequired": "Credential expired",
        "accountUnavailable": "No account selected on the remote",
      ...
      "desktopCurrent": "Current Desktop Codex account",
      "credentialReady": "Remote credential ready",
      "credentialExpired": "Remote credential expired",
      "credentialMissing": "Not authorized on remote",
    ...
    "control": {
      ...
      "choose": "Choose a Remote Control account",
      "credentialExpired": "This account's remote credential has expired. The identity still matches, but model requests and Remote Control authentication can fail.",
    ...
    "execution": {
      "title": "Official model account",
      "identity": "Model requests use",
      "followControl": "Follow the Remote Control account",
      "independentHint": "This only chooses the account and quota used for official model requests. It does not change the Remote Control identity.",
      "inheritedCredentialExpired": "Model requests currently follow the Remote Control account, but its remote credential has expired. Sign that account in again or select an independent model account.",
      "empty": "No Vellum-managed Official execution account.",
      "selected": "Selected",
      "select": "Select",
      "remove": "Remove",
      "removeSelected": "Remove the selected independent account",
