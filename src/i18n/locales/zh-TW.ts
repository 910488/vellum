...
const zhTW: ResourceTree = {
  ...
  "models": {
    ...
    "ui": {
      ...
      "pool": { "title": "額度池", "onHint": "池裡的帳號自動接手；每個帳號可以保留一段每週額度。", "offHint": "關閉時維持手動切換，新請求使用你選定的帳號。", "rules": "判定規則", "rulesTitle": "額度池判定規則", "strategyLabel": "輪替順序", "strategy": { "rank": "依順位", "most": "餘量最多優先", "soonest": "最快重置優先" }, "rankHint": "左邊的號碼就是順序，用 ▲▼ 調整。", "computedRankHint": "號碼是這個策略算出的順序。", "availableThisWeek": "這週還可自動取用", "availablePercent": "池內這週還可自動取用 {{value}}%", "currentAndNext": "現在由 {{current}} 取用，接著是 {{next}}。", "stalled": "池內沒有可取用的帳號；不會越過閘門或使用池外帳號。", "empty": "池是空的。從下面的帳號按「加入池」才會開始自動輪替。", "using": "取用中", "next": "下一個", "standby": "待命", "outside": "未加入池", "pause": "暫停", "resume": "恢復", "add": "加入池", "remove": "移出池", "moveUp": "往前一個順位", "moveDown": "往後一個順位", "weeklyWindow": "每週窗", "fiveHourWindow": "5 小時窗 · 上游節流，不設閘門", "burnable": "可燒 {{value}}%", "atGate": "已到閘門", "leftAndGate": "剩 {{left}}% · 閘門 {{floor}}%", "fiveHourEmpty": "用完了", "fiveHourLeft": "剩 {{value}}%", "gateLabel": "{{account}} 的每週窗閘門", "gateValue": "閘門 {{floor}}%，這週剩 {{left}}%", "saveFailed": "無法儲存額度池：{{detail}}", "reason": { "paused": "已暫停", "weeklyGate": "每週窗到閘門", "fiveHour": "5 小時窗用完", "missingQuota": "額度資料不完整" }, "rule": { "gate": { "title": "閘門是地板", "body": "燒到每週剩餘量等於閘門就停手；以下留給你自己，100% 就完全不給自動輪替使用。" }, "fiveHour": { "title": "5 小時窗唯讀", "body": "它是上游節流；歸零時暫時輪不到，重置後自動回來。" }, "strategy": { "title": "順序可解釋", "body": "依順位可手動排序；其他策略由最新額度讀數計算。" }, "pause": { "title": "暫停與移出", "body": "暫停保留設定但暫不使用；移出後自動輪替不會碰它。" }, "reset": { "title": "不自動花 Reset", "body": "額度池不會自動消耗 Reset；手動重設後閘門維持原值。" }, "empty": { "title": "無帳號可用", "body": "所有池內帳號受限時明確停止，不越過閘門，也不使用池外帳號。" } } },
