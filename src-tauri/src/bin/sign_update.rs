...
//!
//! Usage: vellum-sign-update <manifest.json> <out.sig> <expected-public-key-hex>
//! Reads the private seed from VELLUM_UPDATE_SIGNING_KEY so it is never a
//! process-list-visible command argument.
...
fn main() {
    ...
    let key_hex = std::env::var("VELLUM_UPDATE_SIGNING_KEY")
        .expect("VELLUM_UPDATE_SIGNING_KEY must contain a 32-byte signing seed as hex");
