...
pub async fn trigger_codex_oauth_five_hour_window(
    ...
    let manager = state.codex_oauth();
    crate::five_hour_cadence::trigger_account(&manager, account_id)
        .await
...
mod tests {
    ...
    fn five_hour_trigger_is_minimal_and_non_persistent() {
        let body = crate::five_hour_cadence::trigger_body();
        assert_eq!(body["model"], crate::five_hour_cadence::TRIGGER_MODEL);
